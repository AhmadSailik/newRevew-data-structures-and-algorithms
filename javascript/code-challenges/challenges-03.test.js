...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let newArr=arr.filter(item=>item.baseStat>minBaseStat).map(item=>item.stat.name);
  return newArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  let newArr=arr.filter(item=>item.children==null);
  return newArr;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let newArr=arr.filter(item=>typeof item=="number").map(item=>{
    if(item%2){
      return "odd"
    }else{
      return "even"
