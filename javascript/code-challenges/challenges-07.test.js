...
const listFoods = (recipe) => {
  ...
  for(let i=0;i<recipe.ingredients.length;i++){
    let num=recipe.ingredients[i].indexOf(" ");
    let str=recipe.ingredients[i].slice(num+1,recipe.ingredients[i].length)
    num=str.indexOf(" ");
    str=str.slice(num+1,str.length)
    result.push(str)
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  for(let i=0;i<recipe.ingredients.length;i++){
    let newStr=recipe.ingredients[i].split(" ").splice(2,recipe.ingredients[i].length).join(" ");
    result.push(newStr)
  }
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  // Solution code here...
  result.push(recipe.ingredients[i].split(" ")[0])
  return result;
};
